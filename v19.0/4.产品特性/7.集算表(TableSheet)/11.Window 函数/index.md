...
窗口函数为在集算表中进行数据分析提供了一组函数。这些函数在与当前行存在某种关联的窗口（一组表格行）上执行计算，并生成一列来显示结果。进行函数求值的行称为当前行。
窗口函数在特定的窗口上执行聚合、排名和分析函数，并为每一行生成一个结果。

> **注意事项**：
>
> * 窗口函数的结果可能会对集算表的行顺序产生影响。
>     如果集算表已经进行了排序或分组，结果将保持不变。
> * 当集算表的分层数据中存在树状结构时，窗口函数将不起作用。
> * 集算表筛选器在窗口函数求值之后执行，并且仅筛选特定的行。
