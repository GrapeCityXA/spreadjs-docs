 ...
 **savePDF** 方法的 **Workbook 类** 可用于在处理表单时执行导出PDF操作。

在SpreadJS中，用户可以使用 **printInfo** 对象根据工作簿中指定的打印设置执行自定义PDF导出。 对于每张表单，用户可以通过指定表单的printInfo选项来设置详细选项。
...
用户还可以使用 **printInfo** 对象的 **blackAndWhite** 属性将表单导出为黑白PDF文件。
