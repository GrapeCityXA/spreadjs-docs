 ...
 **示例** -下图描绘了显示不同公司智能手机的每日销售记录的图表。 您可能会注意到，表单中隐藏了第三行以及C和D列。 可见数据是2家公司（Apple和LG）的4个日期的数据，而图表显示了3家公司（同样是三星）的6个日期的数据。 要显示图表中隐藏行和列的数据，请将 **ignoreHidden** 函数的值设置为true，将显示您的图表，如以下截图所示。
