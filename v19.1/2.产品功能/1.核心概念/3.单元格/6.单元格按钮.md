...
单元格按钮是一组预定义的按钮，可以添加到工作表单元格中以配置额外功能。它们使您能够为诸如用户点击单元格按钮、编辑单元格或鼠标悬停在单元格上之类的操作配置特定行为。[cellbuttons ](gcdocsite__documentlink?toc-item-id=1a4e9265-6694-48bb-b0fc-15e5ab238a26#cellbuttons)类型属于 [Style ](gcdocsite__documentlink?toc-item-id=1a4e9265-6694-48bb-b0fc-15e5ab238a26)类，因此它允许您创建命名样式，这些样式可以有选择地轻松应用于任何单元格。
例如，您可以通过为单元格定义一个自定义的颜色选择器命名样式，来创建一个带有颜色选择器下拉菜单的按钮。此样式可以重复使用并应用于任何其他单元格。

## 单元格按钮

以下动图展示了一些单元格按钮命令和属性的用法，以实现所需的自定义效果。

* 带有“插入”标题的单元格按钮
    ![basic-cellbutton](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/basic-cellbutton.50c087.gif)
    ...
    activeSheet.setText(2, 3, '带有标题的基本按钮');
    activeSheet.setStyle(2, 4, basicButttonStyle);
    
    ```
* “enabled”属性设置为 true 和 false 的单元格按钮
    ![enable-disable-cellbutton](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/enable-disable-cellbutton.0ffdb5.gif)
    ...
    activeSheet.setText(4, 3, '启用/禁用按钮');
    activeSheet.setStyle(4, 4, customButtonStyle);
    
    ```
* 设置为在选择单元格时可见的单元格按钮
    ![cellbutton-visible](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/cellbutton-visible.f4ce3a.gif)
    ...
    activeSheet.getCell(10,10).value("选择单元格..").vAlign(GC.Spread.Sheets.VerticalAlign.bottom);
    ```
* 带有内置和自定义图像的单元格按钮
    ![image-cellbutton](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/image-cellbutton.e379f1.gif)
    ...
    activeSheet.setText(6, 3, '图像按钮');
    activeSheet.setStyle(6, 4, ImageStyle);
    
    ```
* 单元格按钮设置命令函数以在工作表中执行放大和缩小操作
    ![cmd-func-cellbutton](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/cmd-func-cellbutton.cc4e88.gif)
    ...
    activeSheet.setText(10, 3, '带有命令函数的按钮');
    activeSheet.setStyle(10, 4, buttonCommandfunction);
    
    ```
* 单元格按钮设置命令函数以触发警告消息
    ![cellbutton-alert](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/cellbutton-alert.a39cae.gif)
    ...
    ```
* 单元格按钮设置为下拉 ButtonImageType 枚举值和 opencolorpicker 命令
    ![cmd-action-dropdown-cellbutton](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/cmd-action-dropdown-cellbutton.a02272.gif)
    ...
    activeSheet.setText(8, 3, '带有命令的下拉按钮');
...
| ButtonImageType | 该属性是ButtonImageType枚举类型，指定单元格按钮的图像类型。您可以根据特定偏好配置按钮图像类型，例如下拉、搜索、撤消、重做。完整列表请参考[ButtonImageType 枚举](gcdocsite__documentlink?toc-item-id=07b2a68f-cac4-4ad2-8b17-a9af69ac270d)。 |
...
| visibility | 此属性指定按钮的可见性。 提供三个选项("always", "onselect" 和 "edit")。 |
