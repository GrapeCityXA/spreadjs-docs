...
在客户端，可通过 `import` 和 `export` 方法将 Excel 文件（.xlsx、xltx、.xlsm、.xltm）导入为 JSON 格式，或将 JSON 对象导出为 Excel 格式。
...
可通过 `import` 和 `export` 方法将 Excel 文件（.xlsx、xltx、.xlsm、.xltm）导入为 Spread 对象，或将 .SJS 文件导入为 Spread 对象，并导出为 Excel 格式。
