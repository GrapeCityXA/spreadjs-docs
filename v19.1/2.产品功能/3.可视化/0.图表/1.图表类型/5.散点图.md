# CreatingScatterChart

散点图被广泛用于展示各个项目或类别之间的关系。这种图表非常适合展示科学、统计和工程数据的比较情况。
工作表中按列或行排列的数据可以绘制在散点图中。与其他图表不同，散点图在绘图区域中分别在水平轴（也称为 X 轴）和垂直轴（也称为 Y 轴）上显示 x 和 y 变量的实际值。通常，这种图表将 x 和 y 值组合成单个数据点，并以不规则的间隔显示它们。此外，这种图表不使用类别轴，因为水平轴（主轴）和垂直轴（次轴）都是数值轴。
SpreadJS 支持以下类型的散点图。在下面展示的示例中，借助不同类型的散点图描绘了一些男性和女性的身高和体重分布情况。

## 散点图

簇状散点图根据选定的数据范围显示数据点。这可以帮助用户分析和确定 x 变量和 y 变量之间的关系。
下面是一张散点图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/xyscatter.a1ba2e.png)

### 带平滑线和数据标记的散点图

带平滑线和数据标记的散点图会显示一条平滑曲线，该曲线连接特定数据系列中的所有数据点。
下面是一张带平滑线和数据标记的散点图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/xyscattersmooth.2d2156.png)

### 带平滑线的散点图

带平滑线的散点图会显示一条平滑曲线，该曲线连接特定数据系列中的所有数据点，但不显示各个单独的数据点。
下面是一张带平滑线的散点图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/xyscattersmoothnomarkers.6196d5.png)

### 带直线和数据标记的散点图

带直线和数据标记的散点图会显示一条直线，该直线连接特定数据系列中的所有数据点。
下面是一张带直线和数据标记的散点图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/xyscatterlines.6a3aa1.png)

### 带直线的散点图

带直线的散点图会显示一条连接特定数据系列中数据点的直线，但不显示各个单独的数据点。
下面是一张带直线的散点图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/xyscatterlinesnomarkers.5ac780.png)

**示例**
以下代码示例展示了如何在电子表格中添加不同类型的散点图。
...
var scatterWithLineDataArray = dataArray = [["Period", "Zantedeschia", "Celosia", "Calendula"], [0, 0, 0, 0], [1, 2, 1, 1], [2, 6, 1, 2], [3, 6, 1, 2], [4, 10, 2, 2], [5, 11, 2, 2], [6, 13, 2, 3], [7, 14, 2, 4], [8, 15, 3, 5],
[9, 16, 3, 7], [10, 17, 4, 9], [11, 22, 4, 11], [12, 27, 5, 12], [13, 30, 8, 13], [14, 32, 10, 14], [15, 34, 13, 15], [16, 36, 16, 15], [17, 37, 20, 15], [18, 39, 23, 15], [19, 40, 25, 15], [20, 40, 25, 15]];
...
function initSheet(sheet, sheetName) {
    ...
    //为图表准备数据
    var dataArray = [["Period", "Zantedeschia", "Celosia", "Calendula"], [0, 0, 0, 0], [1, 2, 1, 1], [2, 6, 1, 2], [3, 6, 1, 2], [4, 10, 2, 2], [5, 11, 2, 2], [6, 13, 2, 3], [7, 14, 2, 4], [8, 15, 3, 5], [9, 16, 3, 7],
    [10, 17, 4, 9], [11, 22, 4, 11], [12, 27, 5, 12], [13, 30, 8, 13], [14, 32, 10, 14], [15, 34, 13, 15], [16, 36, 16, 15], [17, 37, 20, 15], [18, 39, 23, 15], [19, 40, 25, 15], [20, 40, 25, 15]];
...
气泡图非常适合用于财务数据分析。它展示了散点图的一种变体，其中数据点被气泡所取代，并且通过气泡的大小来表示第三个维度（Z 轴）。这种图表会绘制 z（大小）值以及 x 值和 y 值。通常，当你想要绘制三个数据系列时可以使用这种图表。气泡的大小由第三个数据系列中的值决定。
下面是一张气泡图的图片：
![](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/5fefd5d8-238b-4c13-893b-6f22fcc5b9dd/bubble_chart.26d2f4.png)

### 设置气泡大小比例

`bubbleScale` 参数决定了气泡图中气泡大小的缩放因子。它接受的整数值范围是 **0 到 300**，表示为默认大小的百分比，其中 **100** 为默认值。

**示例**
以下代码示例展示了如何在电子表格中添加一个气泡图。

```javascript
var spread = new GC.Spread.Sheets.Workbook('ss');
var sheet = spread.getActiveSheet();
initBubbleSheet(sheet);
initBubbleChart(sheet);
...
function initBubbleChart(sheet) {
    var chart = sheet.charts.add('BubbleChart1', GC.Spread.Sheets.Charts.ChartType.bubble, 280, 0, 620, 400, 'A1:E38');
...
![image](/DOCUMENT_SITE_LINK_PREFIX_HERE/document-site-files/images/1f04de1b-0add-4f88-889e-295f2a2e218b/image.8ff704.03ca04.png)

> type=warning
> **注意**：对于散点图和气泡图，由于“类别轴标签”选项中的“编辑”按钮已禁用，你只能使用“系列”选项来编辑名称、X 值和 Y 值。有关更多信息，请参阅[编辑系列](gcdocsite__documentlink?toc-item-id=b49eaadd-c1d5-46a0-b532-40519afedfc2)。