...
在使用SpreadJS时，添加悬停动画可以使用户与图表中的数据点进行交互。 此功能可帮助用户浏览数据并将基本图表转换为响应图表。 此外，它增强了图表的可读性，同时使工作表对用户更具吸引力。
悬停动画在以下情况下非常有用：

* 用户有兴趣了解数据点的准确值。
* 用户希望在分析图表时使用鼠标悬停显示其他信息以获得有价值的结果。

以下截图描绘了带有悬停动画的图表。 用户只需在图表上移动鼠标指针，即可在工作表中使用鼠标悬停动画效果。
...
请参考以下示例代码，以便在可视化工作表中的数据时向图表添加悬停动画和悬停样式。
