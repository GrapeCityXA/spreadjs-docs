...
在使用 SpreadJS 包（包括 SpreadJS 设计器组件）构建 Angular 应用程序时，你可能会遇到错误消息“*发生未处理的异常：超出映射最大大小*”。仅当 Angular 应用程序使用本地化功能且使用 ESbuild 打包工具进行构建时，才会出现此问题。出现该问题的原因是 SpreadJS 包采用的是 CommonJS 格式，而 ESbuild 不支持对 CommonJS 模块进行摇树优化（tree-shaking）。