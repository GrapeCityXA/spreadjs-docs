...
对于甘特图工作表的 JSON 序列化和反序列化，你需要使用 **Workbook** 类的 **toJSON** 和 **fromJSON** 方法。甘特图工作表的 JSON 数据存储在 **workbookJSONData.sheetTabs** 中，甘特图工作表的指定属性存储在 **ganttSheetTabData.gantt** 中。此外，在 **fromJSON** 过程中，**sheetType** 属性对于区分集算表（TableSheet）和甘特图工作表（GanttSheet）是必需的。
